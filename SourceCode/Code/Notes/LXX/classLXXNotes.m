...
@implementation classLXXNotes

classGlobal *globalVarsLXXNotes;

- (id) init: (classGlobal *) inConfig
{
    if( self = [super init])
    {
        globalVarsLXXNotes = inConfig;
    }
    return self;
}
...
-(void) storeANoteFor: (NSInteger) bookId chapterSequence: (NSInteger) chapIdx andVerseSequence: (NSInteger) verseIdx
{
    ...
    noteTextView = [globalVarsLXXNotes txtLXXNotes];
    if( [noteTextView string] != nil)
    {
        partialPath = [[NSString alloc] initWithFormat:@"%@/%@/%@", [globalVarsLXXNotes iniPath], [globalVarsLXXNotes notesPath], [globalVarsLXXNotes lxxNotesFolder]];
        notesPath = [[NSString alloc] initWithFormat:@"%@/%@/%ld-%ld", partialPath, [globalVarsLXXNotes specificLXXNoteFolder], bookId, chapIdx];
        ...
        if  ( [[noteTextView string] length] > 0 )
        {
            ...
            fileContents = [[NSString alloc] initWithFormat:@"%@", [globalVarsLXXNotes lxxNotesName]];
            [fileContents writeToFile:[[NSString alloc] initWithFormat:@"%@/%@/NoteSetName.desc", partialPath, [globalVarsLXXNotes specificLXXNoteFolder]] atomically:YES encoding:NSUTF8StringEncoding error:nil];
...
-(void) retrieveANoteFor: (NSInteger) bookId chapterSequence: (NSInteger) chapIdx andVerseSequence: (NSInteger) verseIdx
{
    ...
    noteTextView = [globalVarsLXXNotes txtLXXNotes];
    noteStore = [noteTextView textStorage];
    notesPath = [[NSString alloc] initWithFormat:@"%@/%@/%@/%@/%ld-%ld", [globalVarsLXXNotes iniPath], [globalVarsLXXNotes notesPath], [globalVarsLXXNotes lxxNotesFolder], [globalVarsLXXNotes specificLXXNoteFolder], bookId, chapIdx];
    fmNotes = [NSFileManager defaultManager];
